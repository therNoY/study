...
public abstract class MyForkJoinTask<V> implements Future<V>, Serializable {
     ...
     */
    private int doJoin() {
        int s; Thread t; MyForkJoinWorkerThread wt; MyForkJoinPool.WorkQueue w;
        return (s = status) < 0 ? s :
    ...
    private int doInvoke() {
        int s; Thread t; MyForkJoinWorkerThread wt;
        return (s = doExec()) < 0 ? s :
                ((t = Thread.currentThread()) instanceof MyForkJoinWorkerThread) ?
    ...
    static void rethrow(Throwable ex) {
        if (ex != null)
            MyForkJoinTask.<RuntimeException>uncheckedThrow(ex);
    }
     ...
     * MyForkJoinPool#commonPool()} if not {@link #inMyForkJoinPool}.  While
     ...
     */
    public static boolean inMyForkJoinPool() {
    ...
    public static int getQueuedTaskCount() {
        Thread t; MyForkJoinPool.WorkQueue q;
        if ((t = Thread.currentThread()) instanceof MyForkJoinWorkerThread)
     ...
     * usages of MyForkJoinTasks, at steady state, each worker should
    ...
    protected static MyForkJoinTask<?> pollTask() {
        Thread t; MyForkJoinWorkerThread wt;
        return ((t = Thread.currentThread()) instanceof MyForkJoinWorkerThread) ?
    ...
    /**
     * Returns a new {@code MyForkJoinTask} that performs the {@code run}
    ...
    /**
     * Returns a new {@code MyForkJoinTask} that performs the {@code call}
