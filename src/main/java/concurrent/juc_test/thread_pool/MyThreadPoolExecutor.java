...
public class MyThreadPoolExecutor extends AbstractExecutorService {
    ...
    // runState is stored in the high-sort bits
...
//        handler.rejectedExecution(command, this);
        ...
        for (;;) {
            ...
            for (;;) { // 这里判断是否能加入core
                ...
                if (wc >= CAPACITY ||
                    ...
                    return false; // 这里返回说明此时core 已满 或者 可以扩展的线程也满了
                if (compareAndIncrementWorkerCount(c))
                    break retry;
                c = ctl.get();  // Re-read ctl
                if (runStateOf(c) != rs) // 如果此时线程池的状态变了 重新判断
                    continue retry;
    ...
    /* Predefined MyRejectedExecutionHandlers */
