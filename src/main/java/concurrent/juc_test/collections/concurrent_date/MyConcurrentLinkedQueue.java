...
public class MyConcurrentLinkedQueue<E> extends AbstractQueue<E>
    ...
    public boolean offer(E e) {
        ...
        // 死循环，入队不成功反复入队 创建一个指向tail节点的引用 p用来表示队列的尾节点，默认情况下等于tail节点。
        for (Node<E> t = tail, p = t; ; ) { // t是当前节点尾巴节点的快照 p是寻找真正的最后一个节点的节点
            Node<E> q = p.next;
            if (q == null) {
                // 说明此时p找打了尾节点 就通过cas 将tail 的next 设置成新节点
                if (p.casNext(null, newNode)) {
                    // Successful CAS is the linearization point
                    // for e to become an element of this queue,
                    // and for newNode to become "live".
                    if (p != t) {
                        // 这个时候如果tail的快照t 不是真正的尾巴节点的话 就更新尾巴节点
                        casTail(t, newNode);  // 这里失败的话 说明其他线程已经将设置好了尾部节点 所以失败也ok
                    }
                    return true;
                }
    ...
    public void show(){
        ...
        while ((c = c.next) != null) {
            if (c.item != null && (tail != c)) {
                System.out.print(c.item + "->");
            }

            if (tail == c) {
                System.out.print("tail= " + c.item + "->");
            }
        }
     ...
     * this queue, in the sort that they are returned by the specified
    ...
    public Object[] toArray() {
        // Use MyArrayList to deal with resizing.
    ...
    public <T> T[] toArray(T[] a) {
        ...
        // If won't fit, use MyArrayList version
        ArrayList<E> al = new ArrayList<E>();
