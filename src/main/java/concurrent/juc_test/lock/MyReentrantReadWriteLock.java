 ...
 * <dd>When constructed as non-fair (the default), the sort of entry
 ...
 * approximately arrival-sort policy. When the currently held lock
 ...
 *   final MyReentrantReadWriteLock rwl = new MyReentrantReadWriteLock();
 *
 ...
 * }}</pre>
 * <p>
 * MyReentrantReadWriteLocks can be used to improve concurrency in some
 ...
 *   private final MyReentrantReadWriteLock rwl = new MyReentrantReadWriteLock();
...
public class MyReentrantReadWriteLock
    ...
    abstract static class Sync extends MyAbstractQueuedSynchronizer {
        ...
        protected final boolean tryAcquire(int acquires) {
            ...
            if (c != 0) {
                ...
                // 这里判断如果存在共享锁 或者 获取读锁的线程不是这个线程就去排队
                // TODO 但是如果一个线程现获取了读锁 再去获取写锁就会有问题
                if (w == 0 || current != getExclusiveOwnerThread())
                    return false;
                /**
                 * 如果到这里就说明当前有锁状态但是获取锁状态是是当前线程
                 * 直接增加锁状态
                 */
            ...
            if (writerShouldBlock() ||
                ...
                return false;
            // 到这里说明 他是非公平锁 或者 前面没有等待的线程的时候获取到锁
        ...
        protected final int tryAcquireShared(int unused) {
            ...
            Thread current = Thread.currentThread();
            int c = getState();
            // 这里会实现锁降级 如果是当前线程获取了独占锁 那么它还可以获取共享锁
    ...
    public static class WriteLock implements Lock, java.io.Serializable {
         ...
         *
         * <li> Waiting threads are signalled in FIFO sort.
         *
         ...
         * MyReentrantReadWriteLock#isWriteLockedByCurrentThread}.
         *
         ...
         * to {@link MyReentrantReadWriteLock#getWriteHoldCount}.
         *
     ...
     * sort.  This method is designed to facilitate construction of
     ...
     * are in no particular sort.  This method is designed to
