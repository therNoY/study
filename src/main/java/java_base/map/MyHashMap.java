...
public class MyHashMap<K, V> extends MyAbstractMap<K, V>
     ...
     * java.util.TreeMapTest. Most methods try to use normal bins, but
     ...
     * them in the same relative access/traversal sort (i.e., field
    ...
    /**
     * Holds cached entrySet(). Note that MyAbstractMap fields are used
     ...
     */
    transient int modCount; /* 这个map被修改的次数 */
    ...
    int threshold; /* 数组的实际值 通过预期值除初始因子（0.75）取整 初始化时有设定 */
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                   boolean evict) {
        Node<K, V>[] tab; /* 原来的table */
        Node<K, V> p; /* 要要入的桶的位置的节点 */
        int n; // 插入前链表的长度
        int i;
        // 1.判断是否需要初始化首次插入需要进行扩容操作
        if ((tab = table) == null || (n = tab.length) == 0)
            /* 数组大小 */
            n = (tab = resize()).length;
        // 2.找到要插入的位置
        if ((p = tab[i = (n - 1) & hash]) == null) {
            /* 2.1 元素想要插入桶的位置没有元素可以直接插入 */
            tab[i] = newNode(hash, key, value, null);
        } else {
            /* hash 碰撞 p 原位置的node */
            Node<K, V> e;
            K k;
            // 2.2 如果原来的元素有值判断是否需要替换 key相同的话
            if (p.hash == hash &&
                    ((k = p.key) == key || (key != null && key.equals(k)))) {
                /* 这个时候由于Key 相同 发生元素替换 */
                e = p;
            } else if (p instanceof TreeNode) {
                // 2.3 判断节点类型 如果是 红黑树直接插入
    ...
    final Node<K, V>[] resize() {
        ...
        int oldCap = (oldTab == null) ? 0 : oldTab.length; // 老的桶的长度
        int oldThr = threshold;
        int newCap; /* 新的数组长度 */
        int newThr = 0; /* 新的数组扩展大小 */
        if (oldCap > 0) {
            ...
            newCap = oldThr; /* 之前的数组为空设置数组table的大小为扩展长度 */
        ...
        table = newTab;
        if (oldTab != null) {
            /* 复制原有数组到新的数组操作 会有Rehash 操作 */
            for (int j = 0; j < oldCap; ++j) {
                Node<K, V> e;
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null; /* 释放旧的引用 */
                    if (e.next == null) /* 如果该节点没有链表 hash位置相同的直接设置 */
                        ...
                        do {
                            next = e.next;
                            if ((e.hash & oldCap) == 0) { /* hash 要放到的地址是不是首地址 */
                                if (loTail == null)
                                    loHead = e;
                                else
                                    loTail.next = e;
    ...
    static final class TreeNode<K, V> extends Entry<K, V> {
        ...
        final void split(MyHashMap<K, V> map, Node<K, V>[] tab, int index, int bit) {
            TreeNode<K, V> b = this;
            // Relink into lo and hi lists, preserving sort
